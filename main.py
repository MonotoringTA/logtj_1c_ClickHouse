...
def extract_props(event: str) -> dict:
    props = {}
    for match in re.finditer(r"\b(t:clientID|SessionID|vrs_session)=([^\s,]+)", event):
        key, val = match.groups()
        if key == "vrs_session":
            val = val[:36]
        props[key] = val.strip().strip("'\"")
    if match := re.search(r"\bvrs-session:\s*([^\r\n]+)", event):
        props["vrs-session"] = match.group(1).strip().strip("'\"")[:36]
    return props

def update_op_props(op: dict, props: dict) -> None:
    """Обновляет session и vrs_session в active_ops, если они ещё не заданы."""
    if not op.get("session") and props.get("SessionID"):
        op["session"] = props["SessionID"]
    vrs_val = props.get("vrs_session") or props.get("vrs-session")
    if not op.get("vrs_session") and vrs_val:
        op["vrs_session"] = vrs_val

def extract_context_as_text(event: str) -> str | None:
    pattern = re.compile(
        r',Context=(?:"([^"]*)"|\'([^\']*)\'|([^,\r\n]+))',
        re.DOTALL
    )
...
def extract_context_lines(event: str) -> list[str] | None:
    pattern = re.compile(
        r',Context=(?:"([^"]*)"|\'([^\']*)\'|([^,\r\n]+))',
        re.DOTALL
    )
...
def build_ts(filename: str, meta: dict) -> datetime:
    ...
    month, day, hour = int(base[2:4]), int(base[4:6]), int(base[6:8])
                ...
                if client_id:
                    if event_meta["event_name"] == "VRSREQUEST":
                        active_ops[client_id] = {
                            "session": session_id,
                            "vrs_session": props.get("vrs_session") or props.get("vrs-session") or "",
                            "ts_vrsrequest": ts_event,
                            "events": [{"meta": event_meta, "text": event_text, "ts": ts_event}],
                        }
                        ...
                        if client_id in active_ops:
                            ...
                            op["events"].append({"meta": event_meta, "text": event_text, "ts": ts_event})
                            ...
                            if op.get("session"):
                                ...
                                first_context = None
                                for ev in op["events"]:
                                    ctx = extract_context_as_text(ev["text"])
                                    if ctx is not None:   # фильтруем только None
                                        first_context = ctx
                                        break
                                batch_ops.append({
                                    ...
                                    "vrs_session": op.get("vrs_session", ""),
                                ...
                                for ev_data in op["events"]:
                                    ...
                                    row = {
                                        ...
                                        "context": extract_context_as_text(event_string)
                                    }
...
if __name__ == "__main__":
    ...
    # <<< перед загрузкой очистим текущий набор
    clear_dataset(DATASET_NAME)

    INPUT_DIR = config["paths"]["input_dir"]
    logger.info(f"Начинаем обработку логов из каталога: {INPUT_DIR}")
